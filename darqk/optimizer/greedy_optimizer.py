...
from .very_tight_kernel_environment import VeryTightKernelEnvironment
...
class GreedyOptimizer(BaseKernelOptimizer):
    ...
    def __init__(self, initial_kernel: Kernel, X: np.ndarray, y: np.ndarray, ke: KernelEvaluator, env = "TIGHT", bw_possible = 1):
        ...
        if env == "VERYTIGHT":
            #from . import WideKernelEnvironment
            self.mdp: VeryTightKernelEnvironment = Environment.make('VeryTightKernelEnvironment', initial_kernel=self.initial_kernel, X=X, y=y, ke=ke, bw_possible = bw_possible)
