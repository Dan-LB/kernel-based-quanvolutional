...
my_allowed_operations = ['II', 'IX', 'IY', 'IZ', 'XX', 'XY', 'XZ',  'YY', 'YZ', 'ZZ']

# mancano XX etc



class TightKernelEnvironment(Environment):
    """
    ...
    def __init__(self, initial_kernel: Kernel, X: np.ndarray, y: np.ndarray, ke: KernelEvaluator, bw_possible = 1, convert_to_int = False):
        ...
        #self.bws = [i/bw_possible for i in range(1,bw_possible+1)]
    ...
    def unpack_action(self, action):
        ...
        """
        generator_index = int(action % len(my_allowed_operations))
        action = action // len(my_allowed_operations)
        ...
        return {'generator': my_allowed_operations[generator_index],
...
TightKernelEnvironment.register()
