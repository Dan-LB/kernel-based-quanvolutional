...
def init_kernel_machine(
    ...
    if args["quantum"]:
        ...
        if args["unsup"]:
            ...
            if use_custom_kernel == True:
                #simulator = Aer.get_backend('aer_simulator')
                
                #kernel = transpile(kernel, simulator)
                #print("Use_custom = True\n")
                #print("USE_CUSTOM_KERNEL = FALSE, riga 161, in util.py")
                return OneClassQSVM(args, use_custom_kernel = True, kernel=kernel)
            else:
                 OneClassQSVM(args)
        print(
    ...
    if args["unsup"]:
        ...
        print("Selecting rbf kernel")
        args["feature_map"] = "rbf"
